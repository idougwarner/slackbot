...
const getAccounts = async (connection) => {
	try {
		const { records } = await connection.query(
			"SELECT Id, Name FROM Account WHERE Stage__c != 'Terminated' AND (RecordTypeId = '0123i0000005ixjAAA' OR RecordTypeId = '0123i0000005nDdAAI' OR RecordTypeId = '0123i0000005iQmAAI')"
		);
		return records
			.map((record) => ({
				label: record.Name,
